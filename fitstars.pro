; $Id: fitstars.pro, v 1.1 Jun 2000 e.d. $
;
;+
; NAME:
;	FITSTARS
...
;	"input PSF" option is applied. Common interpolation techniques (Fourier
;	transform shift, spline interpolation) cannot be applied to undersampled
;	data. In this case suitable techiques should be used instead, exploiting
;	the "PSF model" option. For details on supported PSF models, see the
;	function IMAGE_MODEL in the file 'image_model.pro'.
;
...
;       4) Make NO_SLANT = 1
;          fit a simple additive offset instead nothing.
;          In the original version, NO_SLANT = 1 switches off any
;          kind of local background fitting (Rainer Schoedel, March 2014)
;-
	...
	for  n = 0L, nstar - 1  do begin
	   (*data).psf_stack[*,*,n] = $
	      image_model(x[n], y[n], unit_flux, sx, sy, (*data).psf, $
	      			  psf_data, INTERP_TYPE = (*data).interp_type)
	   model = model + f[n] * (*data).psf_stack[*,*,n]
	endfor
	bplane = 0
	if  b_nterms eq 3  then  begin
         bplane = plane(b[0], b[1], b[2], sx, sy)
	 model = model + (*data).fixed_image + bplane
        endif else model = model + (*data).fixed_image + b[0]
	return, model
end
...
PRO fitstars, $
	...
	if  not keyword_set(no_slant)  then begin
	   ...
	   if  n_elements(background) ne 0  then $
	      b[0] = total(background) / n_elements(background)
           ...
           ; uncomment the following line if you want the keyword  
           ; NO_SLANT to work like in original StarFinder,
           ; i.e. fit the star without any kind of additive
           ; offset
	   ;b_nterms = 0  &  b = 0
	   b_nterms = 1  &  b = fltarr(b_nterms) & b[0] = total(background) / n_elements(background)
	endelse
	...
	if  size52(psf, /N_DIM) eq 2  then $
	   tot_psf = total(sub_array(psf, sx, sy))  else $
	   tot_psf = total(image_model(sx/2, sy/2, 1, sx, sy, psf, pdata))
	x = x0  &  y = y0

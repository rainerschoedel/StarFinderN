	...
	case  event_type  of
	   ...
	   'go': begin
	      ...
	      m = get_max((*data).image_out)
              print, "mask width is ",form.width
	      if  mask  then $
	         (*data).image_out = circ_mask((*data).image_out, m[0], m[1], rad, BORDER=form.width)
	      if  comp  then $
...
PRO ximage_support_par, id, maxsiz, par
	...
	endelse
        width = 0
	init = {mask:  mask, $
                rad:   strcompress(string(rad), /REMOVE_ALL), $
                width: strcompress(string(width), /REMOVE_ALL), $
                comp: comp, $
                thresh: strcompress(string(thresh), /REMOVE_ALL)}
...
FUNCTION ximage_support, image, wnum, display_opt, DEFAULT_PAR = par, $
	...
	; Define form
	desc = [ $
	'0, LABEL,Soft-edged Circular mask:', $
	'1, BASE,,COLUMN,FRAME', $
	'0, BUTTON,No|Yes,EXCLUSIVE,TAG=mask,ROW', $
	'0, INTEGER,,LABEL_LEFT=Circular mask radius [pixels]:,TAG=rad', $
	'0, INTEGER,,LABEL_LEFT=Width of cosine-shaped edge [pixels]:,TAG=width', $
	'2, LABEL,(The mask is 1 from 0 to r; and 0 from r+w to infinity)', $
	...
	'1, BASE,,ROW', $
	'0, BUTTON,Processing...,NO_RELEASE,TAG=go', $

...
PRO nofinder, $
	...
	SV_SIGMA_R = sv_sigma_r, SV_SIGMA_A = sv_sigma_a, $
   	x, y, fluxes, sigma_x, sigma_y, sigma_f, correlation, STARS = stars, $
        LOGFILE = logfilename, XIN = xin, YIN = yin, FIN = fin
...
;print, "SV_SIGMA_R = ",sv_sigma_r,", SV_SIGMA_a = ",sv_sigma_a
	...
	; central intensity > known stars + background + threshold
        ;
        ; HERE: Stars are pre-defined, just give some feedback.

	if  not keyword_set(silent)  then $
	   print, "NOFINDER: pre-defined stars"
        x0 = xin
        y0 = yin
        i0 = 0.01*fin
        n_max = n_elements(x0)
	...
	list_of_stars = merge_list(list_of_stars, sort_list(list_of_max))
        list_of_stars[*].is_a_star = 1B
        n_stars = n_max
	list_of_stars = sort_list(extract_stars(list_of_stars, n_stars))
        if  logfilename ne ""  then printf, logfp, n_max, " pre-defined stars"
;	if  not keyword_set(silent)  then $
...
;	   if  not keyword_set(silent)  then $
;	      print, "NOFINDER: analysis of suspected stars"
;	   list_of_max = create_list(n_max, x0, y0, i0)
;	   list_of_stars = merge_list(list_of_stars, sort_list(list_of_max))
;
...
;               if  not keyword_set(silent)  then $
;                 print, "NOFINDER: Update background estimate"

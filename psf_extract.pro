; $Id: psf_extract.pro, v 1.1 Dec 1999 e.d. $
;
;+
; NAME:
;	PSF_EXTRACT
...
;		of 1/2 pixel.
;
;	NORM_MAX:	Set this keyword to normalize the stars to unitary maximum
;		before the median superposition. The default is to normalize each
;		sub-image to total unit flux.
;		Note:	as far as I understand the code, the final psf will be
;			normalized to total unit flux in the end, RK 16may01
;
;	UNWEIGHTED:	Set this keyword to avoid weighing the stars in the median
;		superposition. The default is to apply a weight proportional to the
;		square root of the original intensity of each PSF star: this weight
;		is proportional to the signal-to-noise ratio in the photon noise case.
;		Notice that the preliminary estimate of the PSF, used to fit and
;       subtract secondary sources, is unweighed by default.
;
; OUTPUTS:
...
PRO psf_extract, $
	...
	ITER = iter, UPPER_LEVEL = upper_lev, _EXTRA = extra
...
print, y
	...
	if  n_satur ne 0  then begin
	   ...
	   x = append_elements(x_satur, x_unsat)
	   y = append_elements(y_satur, y_unsat)
	   psf = superpose_stars(clean_image - background, x, y, psf_size, $
	   			 SATURATION = upper_lev - background, _EXTRA = extra)
	   psf_fwhm = fwhm(psf, MAG = 3, /CUBIC)
	endif

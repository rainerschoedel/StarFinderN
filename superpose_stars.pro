; $Id: superpose_stars.pro, v 1.2 Feb 2000 e.d. $
;
;+
; NAME:
;	SUPERPOSE_STARS
...
;	centered on the stars and combine them by median superposition. This
;	routine may be used to estimate the PSF in a stellar field image.
...
;	the other point sources around before median superposition.
;
; CATEGORY:
;	Signal processing.
;
...
;
;	UNWEIGHTED:	Set this keyword to avoid weighting the stars in the median
;		superposition.
;
...
;	3) The extracted stellar images are normalized before median
;	superposition (see the keyword NORM_MAX for details). Saturated stars
;	should have been previously repaired to ensure proper normalization.
;	However previous repair of saturated stars should not prevent the use
;	of the keyword SATURATION.
;
; PROCEDURE:
...
;	masked.
;	Compute weights for median superposition, as the square root of the
;	peak of each star. Center each sub-image with sub-pixel accuracy,
...
;	extracted.
;	Combine the frames in the stack by median superposition.
;
...
FUNCTION superpose_stars, image, x, y, siz, stack, masks, _EXTRA = extra, $
   						  ...
   						  SATURATION = satur_lev, NORM_MAX = norm_max, $
   						  UNWEIGHTED = unweighted, STARS = stars, PSF = psf, $
	...
	if  nframes gt 1  then $
	   combined = stack_median(stack, WEIGHTS = w, MASK = masks)  else $
	   combined = stack[*,*,0] * masks[*,*,0]

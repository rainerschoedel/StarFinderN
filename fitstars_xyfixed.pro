...
FUNCTION fs_model, parameters, DATA = data, PSF_DATA = psf_data
    ...
    fs_deparam, parameters, f, b, nstar, b_nterms
    x = (*data).x  &  y = (*data).y
...
FUNCTION fs_param, f, b, nstar, b_nterms

    parameters = fltarr(nstar + b_nterms)
    parameters[0] = f
    if  b_nterms ne 0  then  parameters[nstar] = b
    return, parameters
end

; FS_DEPARAM: convert program parameters vector to user parameters.

PRO fs_deparam, parameters, f, b, nstar, b_nterms

    f = parameters[0:nstar-1]
    if  b_nterms ne 0  then  b = parameters[nstar:*]
    return
end
...
FUNCTION fs_iacobi, parameters, DATA = data
    ...
    nstar = (*data).nstar  &  b_nterms = (*data).b_nterms
    n_pix = sx * sy
    iacobi = fltarr(n_pix, nstar + b_nterms)
    for  n = 0L, nstar - 1  do begin
       iacobi[*,n] = reform((*data).psf_stack[*,*,n], n_pix)
    endfor
    if  b_nterms ne 0  then  iacobi[*,nstar] = 1
    if  b_nterms eq 3  then begin
       iacobi[*,nstar+1] = reform(plane(0, 1, 0, sx, sy), n_pix)
       iacobi[*,nstar+2] = reform(plane(0, 0, 1, sx, sy), n_pix)
    endif
...
FUNCTION fs_convergence, p0, p, DATA = data
    ...
    fs_deparam, p0, f0, b0, (*data).nstar, (*data).b_nterms
    fs_deparam, p,  f,  b, (*data).nstar, (*data).b_nterms
    check = convergence(f0, f, (*data).phot_tol)
...
PRO fs_out, it, fit_error, parameters, sigma_p, nstar, b_nterms
    ...
    if  n_elements(sigma_p) ne 0  then $
       fs_deparam, sigma_p, sigma_f, sigma_b, nstar, b_nterms
...
PRO fitstars_XYfixed, $
    ...
    p0 = fs_param(f, b, nstar, b_nterms)
    ...
    if  converged  then begin
       ...
       fs_deparam, p, f, b, nstar, b_nterms

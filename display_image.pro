; $Id: display_image.pro, v 1.1 Nov 1999 e.d. $
;
;+
; NAME:
;	DISPLAY_IMAGE
...
;	2) changed sqrt-scaling to subtract user-set lower limit
;	   before sqrt.  Rainer Koehler, Oct.2001
;-
;      (3) changed  s = s / max(s) * smax to  s = float(s) / max(s) * smax
;          to deal with odd window sizes
;          (Rainer Schoedel, June 2012)
;      (4) introduced options.large (see default_display_opt.pro)
;          for larger image on screen 
;          (Rainer Schoedel, July 2014)
...
PRO display_image, image, wnum, OPTIONS = options, MODIFY_OPT = modify_opt
	...
	if  n_elements(wnum) eq 0  then begin
	   if (options.large gt 0) then begin
            smax = get_screen_size()  &  smax = min(smax)*options.large 
           endif else begin
            smax = get_screen_size()  &  smax = min(smax)*2./3.
           endelse
	   s = size52(image, /DIM)  &  s = float(s) / max(s) * smax  &  s = round(s)
	   window, /FREE, XSIZE = s[0], YSIZE = s[1]  &  wnum = !D.window
print, 'Window size: ' + strn(s)
	endif
	...
	w = where(data ge options.chop, count)
	if  count ne 0  then  data[w] = min(data)
	case options.stretch of
	   'square': data = data^2
	   'linear': ; do nothing
	   'square root': data = options.range[0] + sqrt((data - options.range[0]) > 0)
	   'logarithm': data = alog10(data > max(data) * 1e-15)
	endcase
	data = !D.table_size * (data - min(data)) / (max(data) - min(data))
	if  options.reverse  then begin
	   data = -data  &  data = data - min(data)
	endif
	; display image
;        print,  'Color table number : ' + string(options.color_table)
	wset, wnum  &  erase, wnum  &  loadct, options.color_table, /SILENT
	tv, congrid(data, !D.x_size, !D.y_size)

...
;	spatially uniform.
;
;       This script is based on image_background, but will trat
;       negative values as bad pixels that will be repaired.
;
;
;
; CATEGORY:
...
;       4) Introduced treatment of negativities as bad pixels.
;
;-

FUNCTION image_background_noneg, image, sampling_step, $
...
;           print, "image_background: i=",i,", j=",j
	   catch, error
	   ...
	   endelse
        ...
        ; Identify negativities (which are non-physical)
        ; and repair them via replace_pix.pro
        neg = where(b lt 0,count)
        if (count gt 0) then begin
           xy = array_indices(b,neg)
           xbad = xy[0,*]
           ybad = xy[1,*]
           b = replace_pix(b,xbad,ybad)
        endif

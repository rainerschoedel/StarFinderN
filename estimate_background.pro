...
;       Modified:
;         1) return shift(b,1,1) - to remedy shift of backgroudd compared to real image
;            Rainer Schoedel, July 2023
;         2) Introduced POSBG keyword to force use of IMAGE_BACKGROUND_NONEG
;            Rainer Schoedel, May 2024
;-

FUNCTION estimate_background, image, step, SKY_MEDIAN = sky_median, POSBG = posbg, SHIFTBG = shiftbg, _EXTRA = extra

        SetDefaultValue, posbg, 0
        SetDefaultValue, shiftbg, 0
	...
	if  keyword_set(sky_median) then begin
            ...
            if (posbg gt 0) then begin
              b = image_background_noneg(image, step, _EXTRA = extra)
              print, "estimate_background using image_background_noneg"
            endif else begin
        ...
        endelse
        if (shiftbg gt 0) then b = shift(b,1,1)
	return, b
end
